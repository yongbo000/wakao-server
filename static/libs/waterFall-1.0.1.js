...
waterFall.prototype = {
    ...
    append: function(data) {
        var host = 'http://bcs.duapp.com/imgs00/';
        var ytemp = new yTemp();
        var _self = this;
        for(var i=0; i < data.length; i++) {
            var tempViewId = this.view;
            var itemHtml = ytemp.parse2HTML(tempViewId, data[i]);
            //添加到高度最小的那一列
            _self.minHeightColumn2().append($(itemHtml));
            /*
            imgReady(host + data[i].pic_url,function() {
                $(itemHtml).find('.img').html(this);
            });*/
        }
        _self.minColHeight = _self.minHeightColumn2().height();
        return this;
    },
    //向服务器请求数据
    getData: function() {
        ...
        yAjax.get(reqUrl, paras, function(response) {
